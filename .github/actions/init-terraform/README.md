...
| `backend-config-overrides` | Multiline string of `key=value` pairs (one per line). Each non-blank line is appended as an additional `-backend-config="<line>"` flag to the `terraform init` invocation. Use for dynamic backend values (per-team bucket, dynamic key, per-environment region) that cannot be baked into the file referenced by `backend-config-path`. Empty by default. | no | `''` |
...
- **Malformed `backend-config-overrides` line** — a line without `=` is passed through unchanged; Terraform surfaces `Invalid -backend-config option` and exits non-zero. Resolution: ensure each non-blank line is `key=value`.
